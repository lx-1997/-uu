...
export function buildCollaborationPrompt(
  ...
  return [
    ...
    "- **delegate** (board_openclaw_delegate)：委派——确认可行后交付执行",
    "",
    "### 委派行为规则（IMPORTANT）",
    "1. ALWAYS 先 assess 再 delegate。assess 返回 confidence < 0.5 时，用本地工具执行。",
    ...
    "7. NEVER 把简单的 device_exec 任务委派给 OpenClaw——直接执行更快。",
    "三者共享会话，不必重复背景。",
