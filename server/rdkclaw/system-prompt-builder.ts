...
export function buildPersonaPrompt(persona: PersonaProfile) {
  ...
  if (persona.delegationBias === "local-first") {
    lines.push(
      ...
      "**速度优先补充**：assess 通过不代表必须 delegate。若 RDKClaw 已确认本地命令链且预计 1-2 步可闭环，应优先本地直接完成；仅当板端明显更快、强依赖板端技能/会话、或本地已出现多轮试错时再委派。",
  ...
  const openClawCollaborationSection =
    delegationBias === "local-first"
      ...
      : [
          ...
          "委派 OpenClaw 前：先做 RDKClaw 本地速度评估（本地是否可 1-2 步闭环、是否已确认命令可直跑）；仅在板端明显更优时再 assess→delegate。delegate 前把任务边界、已执行证据、失败模式、验收标准与约束写入 guidance/context，减少板端反复试探。",
...
function buildCollaborationPromptLocalFirst(boardSnapshot: BoardSnapshot, tier: ModelTier): string {
  ...
  if (tier === "small") {
    return [
      ...
      "- **多步/技能**：先判断本地是否更快收敛；仅在板端明显更优时 assess→delegate，勿用大量 shell 硬顶。",
...
export function buildCollaborationPrompt(
  ...
  if (tier === 'small') {
    return [
      ...
      "## 协作（简版）",
      "（三条链与工具边界见 **工具契约总纲**。）",
      "你与板端 OpenClaw 是协作双引擎：你负责编排与外部信息，板端负责现场执行与会话迭代。",
  ...
  return [
    ...
    "",
    "### 委派行为规则（IMPORTANT）",
    "1. 先做 RDKClaw 本地速度评估；若板端明显更优再 assess→delegate。`board_openclaw_assess` 对若干 RDK 标准例程会短路由返回 JSON（不跑板端 LLM）；assess 返回 confidence < 0.5 时，退回本地/SSH 执行。",
