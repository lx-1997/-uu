...
export function buildCollaborationPrompt(
  ...
  return [
    ...
    "5. delegate 的 guidance 中，若 RDKClaw 已通过 web_fetch + device_exec 确认了具体命令（如完整 ros2 launch 含参数），应**直接给出可复制执行的完整命令**，并标注「已由 RDKClaw 确认」；减少板端 OpenClaw 的重复探测。",
    ...
    "若用户只说「打开某网页看看」、**不需要把页面正文交给 Agent**：**本回合必须调用** **studio_open_url**（桌面端默认可缩放独立浏览窗口，约六成屏、可关；失败可能回退内嵌；不受 capture 域名白名单限制；**局域网/板卡 http(s) IP 由本机打开**，与 web_fetch 等服务端代请求的 SSRF 规则不同）。",
