...
export function buildToolContractQuickOverviewPrompt(): string {
  return [
    ...
    "板端多步/技能链用 `board_openclaw_assess`→`delegate`（须 **网关 + Studio↔板**；要 apt/API 还须 **板端出网**）；**文档已确认的单条 launch** 优先 `device_exec` 后台跑通（~60s 验收），勿无谓委派。**OpenClaw 未装时**：RDKClaw 须同样「文档+探测+有计划短链+验收」，持久 shell 可延续 source；纯 SSH 不能替代 SkillHub 技能链。**自建多步 SSH 时**：对用户说明须含编号计划+阶段归纳+验收标准，**禁止**只贴日志。",
