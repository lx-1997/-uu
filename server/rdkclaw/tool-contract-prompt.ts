...
export function buildToolContractOverviewPrompt(): string {
  return [
    ...
    "",
    "### 典型顺序（软约束，按任务裁剪）",
    "- 缺资料 → `web_search` / `web_fetch`（**可与 `board_openclaw_assess` 同轮并行**，见协作节——不要等 SSH 全跑完才想起评估板端）。**RDK 标准例程类任务见上节「官方文档优先」。**",
    ...
    "- **视觉/相机/检测/推理类任务**：在按文档 `source` + `ros2 launch` 或设置 **`CAM_TYPE`** 等参数**之前**，先用少量 `device_exec` 确认**当前板端具备何种视频输入**：如 `ls -l /dev/video*`、`v4l2-ctl --list-devices`（若已安装）、`lsusb`、必要时 `dmesg | tail` 看传感器枚举。",
    "- **USB 与 MIPI 不可凭记忆假设**：文档常见 `CAM_TYPE=usb` / `mipi`（或等价项）；须与**实测**一致；若只有其一或节点不符，应先改参、换 launch 或向用户说明缺少硬件/驱动。",
    "- **顺序**：先 **`web_fetch` 文档**弄清变量含义与推荐值 → **再探测** → **再设参启动**；禁止跳过探测直接套用默认相机类型。",
    "",
    "### 失败时（从返回里推断下一步）",
