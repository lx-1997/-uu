...
export function buildForumAuthContextPrompt(): string {
  ...
  if (!anyForum) {
    lines.push("- 当前未检测到论坛凭据；需要发帖时请引导用户使用主账号登录 Studio，或在对话中提供账号密码。");
  } else {
    lines.push(
      "- 涉及发帖/读帖时，必须先调用 forum_drobotics_auth_status。若返回 read_access: granted / auth_status ok，则不要要求用户再提供论坛密码。",
      "- 仅当 auth_status 明确失败或未配置时，再请用户重新登录主账号，或在对话中提供凭据。",
    );
  }
  lines.push(
    "### 用户问「我的社区/论坛用户名、密码是多少」时（必须遵守）",
    "- **禁止**用「这是你的隐私我查不到」一句话打发；本机已配置的信息可以通过工具返回。",
    "- **必须先调用** forum_drobotics_auth_status；用返回里的 `studio_forum_username_masked` 告知用户当前在 Studio 中登记的**脱敏用户名**（与本机 forum-auth 登记一致）。",
    "- **密码**：说明助手**不会也绝不能**在对话里复述明文密码（模型拿不到）；若已主账号同步或本机已保存凭据，说明「发帖/登录由服务端工具自动完成，无需把密码发到聊天里」。",
    "- 仅当工具显示完全未配置时，再引导重新登录主账号或使用论坛找回密码。",
  );
  return lines.join("\n");
}
