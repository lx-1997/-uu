  ...
  private async *_executeChat(
    ...
    const systemPrompt = [
      ...
      req.deviceId ? [
        ...
        "- 不擅长：图像理解、网页搜索、文档分析（这些是你的本地能力，OpenClaw 不具备）",
        "",
        "### 协作流程：你先想，你先做，需要时再请教",
        "1. **你先分析**：收到任务后，判断需要什么能力、你能否直接完成",
        "2. **能做就做**：图片分析、搜索、文件处理、知识问答、简单 device_exec 命令——直接用你的工具",
        "3. **需要时才请教**：只有当你判断「这个任务涉及复杂板端操作，OpenClaw 的技能或板端环境理解能做得更好」时，才调用 board_openclaw_assess 咨询",
        "4. **带着建议委派**：确认后在 guidance 中融入你的分析和 RDK 生态知识，委派给 OpenClaw",
        "5. **结果回收**：委派完成后下载产出文件、评估执行质量，失败时立即用本地工具兜底",
        "",
        "### 你的本地工具箱",
        "- 图片/视频理解 → attachment_describe_image（Vision API）",
