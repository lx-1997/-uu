...
- 对于 RDK / 嵌入式场景，优先保留：设备 ID 或别名、型号、IP/主机、SSH 是否通、套件端 OpenClaw/Agent 版本或安装与否、BPU/部署相关路径与状态、最近一次失败命令或 stdout/stderr 的关键行（可截断但保留头尾特征）。`;
...
## 6. 设备与环境状态
[设备标识、IP/主机、型号、SSH/凭据是否就绪、Agent/OpenClaw/套件端服务状态、重要工作目录]
