---
name: agent-browser
description: Vercel agent-browser CLI：无头/可有头 Chromium，基于无障碍树 snapshot 与 @ref 的稳定点击与填表。适用多步网页自动化、复杂 SPA；需本机或目标环境已安装 `agent-browser`。触发：无头浏览器、网页自动化、agent-browser、snapshot、可访问性树、CLI 浏览器。
version: 0.1.0
trigger: 无头浏览器,网页自动化,agent-browser,snapshot,可访问性树,填表,爬取页面,headless,chromium
risk: medium
