...
- 每轮对话结果必须包含完整链路信息：
  ...
  🎤 ASR 识别结果：<用户语音转文字>
  🤖 AI 回复：<LLM 生成的回复文本>
  🔊 TTS 音频：<生成的音频文件路径，如 /tmp/tts_output.wav>
  ```
- 延迟预期：ASR 约 1-3 秒，TTS 约 2-5 秒（ARM 设备），总轮次延迟应控制在 10 秒内
- 错误降级方案：
  - ASR 失败 → 提示用户检查音频格式/时长，询问是否改用在线 `speech_to_text`
  - TTS 失败 → 先以文字形式返回 AI 回复，询问是否改用在线 `text_to_speech`
  - 环境安装失败 → 输出具体错误日志，建议手动检查 Python 环境和磁盘空间
