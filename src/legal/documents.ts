  ...
  sections: [
    {
      ...
      paragraphs: [
        ...
        '（4）产品改进与统计：在**您或管理员开启**的前提下，我们可能处理匿名或去标识化信息（例如本地随机标识符、应用版本、UTC 日期、事件类型与时间戳、脱敏后的网络地址等），用于估算活跃情况、稳定性与功能改进；**默认不包含聊天全文**。若启用「打开次数」类统计且您通过 SSO 登录，服务端可能将**与对话归档一致的登录展示名**（优先姓名，其次邮箱前缀等）随每次验证/打开写入统计表（**按次计 PV**，非仅每日去重），**不用于广告画像**。无 SSO 的本地部署场景可使用浏览器匿名标识。您可通过关闭服务端统计或相关环境变量限制此类处理。',
  ...
  sections: [
    {
      ...
      paragraphs: [
        ...
        '(4) Product improvement / stats (if enabled): anonymous or de-identified data such as local random ID, app version, UTC date, event types, coarse network info — not chat content by default. For optional open/PV metrics with SSO, we may record the same display name as conversation archives (name, email prefix, etc.) on each verified session/open — not for ads. Non-SSO deployments may use a browser anonymous ID. Disable via server config or environment as applicable.',
