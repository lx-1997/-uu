...
export const TERMS_OF_SERVICE_ZH: { title: string; intro: string; sections: LegalSection[] } = {
  ...
  sections: [
    ...
    },
    {
      heading: '二、服务内容与软件许可',
      paragraphs: [
        '本软件为面向开发者及相关专业人员提供的 RDK 系列开发板及相关开发环境的连接、配置、管理与辅助工具，可能包括但不限于远程连接、文件与命令操作、远程桌面、在线开发环境、ROS 相关能力、智能助手与设备侧 Agent 协同等功能；具体服务范围、界面与能力以实际提供的版本及说明为准，我们可能依技术、合规或商业安排对功能进行调整、升级、优化或下线。',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '您理解开发板固件烧录、远程命令执行、系统配置变更等操作具有固有风险，可能导致数据丢失、设备不可用或安全事故；您应在具备相应能力与授权的前提下谨慎操作，并自行完成必要备份与确认。',
      ],
    },
    {
      heading: '五、用户内容与数据',
