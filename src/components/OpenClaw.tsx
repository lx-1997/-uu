...
export default function OpenClaw() {
  ...
  const handleCheckStatus = () => {
    ...
    setTimeout(() => {
      ...
      addToast('OpenClaw 服务状态已刷新', 'success');
    }, 1500);
  };
  ...
  const toggleService = () => {
    ...
    if (serviceRunning) {
      ...
      setServiceRunning(true);
      addToast('OpenClaw 服务已启动', 'success');
    }
  };
  ...
  const switchModel = (id: string) => {
    ...
    addToast(`已切换到 ${models.find(m => m.id === id)?.name}`, 'success');
  };

  // 添加模型
  const handleAddModel = () => {
    if (!newModel.name || !newModel.key) {
      addToast('请填写模型名称和 API Key', 'warning');
      return;
    }
  ...
  const addCustomSkill = () => {
    ...
    setSkills(prev => [...prev, { id, icon: '🔧', name: newSkillName.trim(), desc: '自定义技能', enabled: true }]);
    setNewSkillName('');
    ...
    <div className="center-stage wide-stage">
      ...
      <div className={`oc-bar ${serviceRunning ? 'live' : ''}`}>
        ...
        <div className="oc-bar-right">
          ...
          <button className={`oc-bar-btn ${serviceRunning ? '' : 'primary'}`} onClick={toggleService}>
      ...
      <div className="isolated-widget" style={{ marginTop: 16 }}>
        <div className="oc-config-bar">
          ...
          <div className="oc-cfg-item">
            ...
            <span className="oc-cfg-val ok">{currentDevice.name}</span>
          </div>
          <span className="oc-cfg-sep" />
          <div className="oc-cfg-item">
        ...
        <div className="panel-card">
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div className="oc-tools-grid">
              ...
              <button className="oc-tl" onClick={() => addToast('日志查看功能开发中', 'info')}>查看日志</button>
              <button className="oc-tl" onClick={() => addToast('性能监控功能开发中', 'info')}>性能监控</button>
              ...
              <button className="oc-tl" onClick={() => { setCmd('openclaw status'); }}>状态诊断</button>
            </div>
          </div>
        </div>
      </div>
